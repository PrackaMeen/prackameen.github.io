  ...
  for (const [kind, orientations] of Object.entries(baseDefinitions)) {
    ...
    normalized[kind] = list.map((entry, index) => {
      ...
      return {
        ...
        animation: normalizeAnimation(entry?.animation) ?? normalizeAnimation(fallbackEntry.animation),
...
function getEntitySpriteSheetSource(kind) {
  return getSpriteSheetSource(getEntityAssetPath(kind));
}
